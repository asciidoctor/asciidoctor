...
If you require more advanced theming, you can inject additional resources using a xref:publish:docinfo.adoc[docinfo file] or a xref:api:postprocessor.adoc[postprocessor extension].
...
|See xref:ROOT:css-and-custom.adoc[custom text styles]
|===
...
|See xref:sections:levels-and-titles.adoc[section levels and titles]
...
|See xref:tables:columns.adoc#cols-align[align column content] or xref:tables:columns.adoc#align-cell[cell content].
...
|See xref:directives:if-directive.adoc#ifeval[ifeval-directive]
...
|See xref:subs:post.adoc[post replacement substitutions]

|Suppress inline substitutions and retain block indents when importing large blocks of plain text
|`plaintext`
|_not implemented_
|Closest Asciidoctor equivalent is a xref:subs:pass-macro.adoc[passthrough block] or a listing block with an indent attribute.
...
See xref:directives:indent.adoc[normalize block indentation] for more detail.
|===
...
In Asciidoctor, you'll need to activate STEM support first using the xref:stem:stem.adoc[stem attribute].

== Configuration files

Asciidoctor does not use [.path]_.conf_ files or filters, so `--conf-file`, `--dump-conf`, and `--filter` are not applicable.
Instead, Asciidoctor provides an xref:api:extensions.adoc[extension API] that replaces the configuration-based extension and filter mechanisms in AsciiDoc.py.
...
|source
a|
* You can choose from a number of xref:source:source-block.adoc[syntax highlighters].
...
|`[latex]` block macro
|Use a xref:stem:stem.adoc#stem-bl[stem block].
...
Depending on the Asciidoctor processor you choose, you can re-write your xref:api:extensions.adoc[extensions in Ruby, Java, or JavaScript].
...
* xref:include:lines-and-tags.adoc[Partial includes]
