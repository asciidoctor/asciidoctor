...
When the `linkcss` attribute is set, the safe mode is safe or lower, standalone output is used, and the target is a file, Asciidoctor will also copy the CSS file to linked destination (unless configured otherwise).
...
If the safe mode is server or lower, standalone output is used, the output target is a file, and the `linkcss` document attribute is set, Asciidoctor will copy the stylesheet to the output directory so the HTML can link to it.

NOTE: The stylesheet is not copied when the output target is not a file or when generating embedded HTML.
