...
////
From src-pygments.adoc, included in:
- user-manual: Source Code Syntax Highlighting: Pygments installation
- {pygments-org} is now {url-pygments}
- {pygments-gem} is now {url-pygments-gem}
- {pygments-lang} is now {url-pygments-lang}
////

{url-pygments}[Pygments^] is a popular syntax highlighter that supports a broad range of {url-pygments-lang}[programming, template and markup languages^].
...
See the xref:publish:html-code-styles.adoc#pygments[Pygments stylesheet section] to learn more about how the value `class` is handled.
Default: `class`.
