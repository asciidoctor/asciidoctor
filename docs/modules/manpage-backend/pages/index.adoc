...
If you want to change `mantitle` and `manvolnum` attributes, you must override the `doctitle` attribute when invoking Asciidoctor.
 ...
 $ asciidoctor --backend=manpage --attribute=doctitle="othername(7)" progname.adoc
...
Now, you can replace the information in the Name section when calling Asciidoctor:
 ...
 $ asciidoctor --backend=manpage \
   --attribute=doctitle="othername(7)" \
   --attribute=manname=othername \
   --attribute=manpurpose="description of othername" \
   progname.adoc
...
However, if you want to make a well-formed man page from a document that doesn't have a Name section, you can effectively insert one by setting the `manname` and `manpurpose` attributes from the CLI:
 ...
 $ asciidoctor --backend=manpage \
   --attribute=doctitle="README(1)" \
   --attribute=doctype=manpage \
   --attribute=manname=README \
   --attribute=manpurpose="Information about this project" \
   README.adoc
...
Another approach is to convert the AsciiDoc document to DocBook using the built-in DocBook converter (e.g., `--backend=docbook`), then convert that document to PDF using the DocBook toolchain.
