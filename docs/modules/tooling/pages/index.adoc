...
All you need to preview an AsciiDoc document in a web browser is to install the {url-chrome-extension}[Chrome extension^], the {url-edge-addon}[Edge add-on^] the {url-firefox-addon}[Firefox add-on^] or the {url-opera-extension}[Opera extension^].
