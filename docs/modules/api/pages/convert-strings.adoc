...
NOTE: When converting standalone output to a file, additional files may be written.
For example, if the `linkcss` and `copycss` attributes are both set, the processor will write the default stylesheet to a file as well.
