...
////
secure-api.adoc, included in:
- user-manual: Running Asciidoctor Securely: Set the safe mode in the API
////

The default xref:attributes:safe-modes.adoc[safe level] in the API is `SECURE`.
...
You can also set the xref:cli:set-safe-mode.adoc[safe mode from the CLI] and xref:attributes:safe-modes.adoc#set-safe-attrs[manage attributes by safe mode].
