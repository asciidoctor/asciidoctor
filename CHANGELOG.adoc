...
Enhancements::
  ...
  * Generate an index in HTML output when an index section is present; add an index term catalog to the document model (#450, PR #4872) (*@dunyakirkali*)
  ...
  * Allow section to override toclevels for itself and descendant sections (#2618)
  ...
  * Skip front matter in included content if skip-front-matter option is set on include directive (#3437)
  * Allow handles? method on include processor to accept doc as first argument; mixin adapter to support old method signature (#2895)
  * Take start value into account when validating explicit numbering on ordered list (#3252)
  * Allow ordered list with explicit numbering to set start value using first item (#2218)
  * Pass through id attribute defined on inline image to output (#4311)
  * Add cxx attribute for C++ as preferred alternative to cpp (#4442)
  * Don't add highlight class to pre tag when `nohighlight` option is set (highlight.js only) (#4804)
...
Compliance::
  ...
  * Change deprecated Droid Sans Mono font to its successor Noto Sans Mono (#4586)
  * Only apply table strips to rows in table body (i.e., tbody) (#3881)
...
Improvements::
  ...
  * Use safe navigation to avoid crashing when querying for extensions (#2973)
...
Bug Fixes::
  ...
  * Retrieve docfile attribute from document for warning message when asset cannot be found or read (#4873)
