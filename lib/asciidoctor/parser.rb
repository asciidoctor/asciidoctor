...
class Parser
  ...
  def self.next_block reader, parent, attributes = {}, options = {}
    ...
    while true # rubocop:disable Lint/UnreachableLoop
      ...
      if !indented && (ch0 ||= this_line.chr) == '<' && CalloutListRx =~ this_line
        ...
        reader.unshift_line this_line
        block = parse_list reader, :olist, parent, style, start: attributes['start']
  ...
  def self.parse_list reader, list_type, parent, style, start: nil
    list_block = start ? (List.new parent, list_type, attributes: { 'start' => start }) : (List.new parent, list_type)
  ...
  def self.parse_list_item reader, list_block, match, sibling_trait, style = nil
    if (list_type = list_block.context) == :dlist
      ...
      when :olist
        ...
        if (start = list_block.attributes['start'].to_i) > 1
          ordinal += start.pred
        end
