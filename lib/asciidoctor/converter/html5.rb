...
#{node.read_asset node.normalize_system_path((node.attr 'stylesheet'), (node.attr 'stylesdir', '')), true}
      ...
      if node.attr? 'icons', 'font'
        ...
        if linkcss
          result << %(<link rel="stylesheet" href="#{node.normalize_web_path @stylesheets.fontawesome_compat_stylesheet_name, (node.attr 'stylesdir', '')}"#{slash}>)
        else
          result << @stylesheets.embed_fontawesome_compat_stylesheet
        end
      end
    ...
    def section node
      ...
      if node.id
        ...
        if node.document.attr? 'sectanchors'
          ...
          #  anchor = %(<a class="anchor" href="##{node.id}"><i class="fa-anchor"></i></a>)
          #else
...
Your browser does not support the video tag.
    ...
    def inline_image node
      ...
      style_type = type == 'icon' ? 'fa' : type
      style_classes = (role = node.role) ? %(#{style_type} #{role}) : style_type
