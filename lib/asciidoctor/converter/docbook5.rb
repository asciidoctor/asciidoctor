        ...
        node.rows[tblsec].each do |row|
          ...
          row.each do |cell|
            ...
            if tblsec == :head
              ...
              when :header
                unless (cell_content = cell.content * '</emphasis></simpara><simpara><emphasis role="strong">').empty?
                  cell_content = %(<simpara><emphasis role="strong">#{cell_content}</emphasis></simpara>)
                end
              else
                unless (cell_content = cell.content * '</simpara><simpara>').empty?
                  cell_content = %(<simpara>#{cell_content}</simpara>)
                end
              end
            end
