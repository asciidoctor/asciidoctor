...
class Asciidoctor::Document
  ...
  def initialize(data, options = {}, &block)
    ...
    while reader.has_lines?
      ...
      @elements << next_block(reader) if reader.has_lines?
    end
  ...
  def build_ulist_item(reader, block, match = nil)
    list_type = :ulist
    this_line = reader.get_line
...
#    item_segment = list_item_segment(reader)
  ...
  def build_ulist(reader, parent = nil)
    items = []
    list_type = :ulist
  ...
  def next_block(reader, parent = self)
