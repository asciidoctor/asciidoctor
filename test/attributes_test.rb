...
....
  ...
  context "Block attributes" do
    test "Section id is generated by default" do
      doc = document_from_string('== Section One')
      assert_equal '_section_one', doc.first_section.id
    end

    test "Section id generation can be disabled" do
      doc = document_from_string("== Section One\n:sectids!:\n")
      assert doc.first_section.id.nil?
    end

    test "Section id can be set explicitly above" do
      doc = document_from_string("[[one]]\n== Section One")
      assert_equal 'one', doc.first_section.id
    end

    test "Section id can be set explicitly inline" do
      doc = document_from_string("== Section One [[one]] ==")
      assert_equal 'one', doc.first_section.id
      assert_equal 'Section One', doc.first_section.name
    end
