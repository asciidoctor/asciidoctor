...
text
  ...
  context 'Docinfo files' do
    ...
    test 'should include docinfo files with substitutions' do
      sample_input_path = fixture_path('subs.adoc')

      output = Asciidoctor.convert_file sample_input_path, :to_file => false,
                                        :header_footer => true, :safe => Asciidoctor::SafeMode::SERVER, :attributes => {'docinfo' => '', 'docinfosubs' => 'attributes, replacements'}
      assert !output.empty?
      assert_css 'script[src="bootstrap.3.2.0.min.js"]', output, 1
      assert_xpath '//meta[@name="title"][@content="Dan & Sarah - Acme™"]', output, 1
    end
  end
