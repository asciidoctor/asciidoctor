...
context 'Substitutions' do
  ...
  context 'Macros' do
    ...
    test 'an icon macro should be interpreted as an icon' do
      ...
      assert_equal %{<span class="image"><img src="./images/icons/github.png" alt="github"></span>}, para.sub_macros(para.source).gsub(/>\s+</, '><')
    end
    ...
    test 'an icon macro should be interpreted as a font-based icon when icons=font' do
      ...
      assert_equal %{<i class="icon-github"></i>}, para.sub_macros(para.source).gsub(/>\s+</, '><')
    end
    ...
    test 'an icon macro with a size should be interpreted as a font-based icon with a size when icons=font' do
      ...
      assert_equal %{<i class="icon-github icon-4x"></i>}, para.sub_macros(para.source).gsub(/>\s+</, '><')
    end
