...
{project-uri}[Asciidoctor] is a _fast_ text processor and publishing toolchain for converting {what-is-asciidoc-uri}[AsciiDoc] content to HTML 5, DocBook 5 (or 4.5) and other output formats.
Asciidoctor is written in {ruby-uri}[Ruby], packaged as a RubyGem and published to {rubygem-uri}[RubyGems.org].
It's also packaged in several Linux distributions, including Fedora, Debian and Ubuntu.
...
We use http://opalrb.org[Opal] to transcompiled the code from Ruby to JavaScript to make {asciidoctorjs-uri}[Asciidoctor.js], which can be used whereever JavaScript runs, such as in a web browser or on Node.js.
...
Asciidoctor reads and parses text written in the AsciiDoc syntax, then feeds the parse tree into a set of built-in templates to produce HTML 5, DocBook 5 or DocBook 4.5.
You have the option of using your own {tilt-uri}[Tilt]-supported templates to customize the generated output or to convert AsciiDoc to additional formats.
...
Asciidoctor also offers a modern, responsive theme based on {foundation-uri}[Foundation] to style the HTML output.
...
Refer to {contribute-uri}[Contributing] to learn how to get involved.

== Installation

Asciidoctor can be installed using the `gem install` command, Bundler or package managers for popular Linux distributions.
...
[TIP]
--
If you install a new version of the gem using `gem install` instead of `gem update`, then you'll have multiple versions installed.
If you wish to remove older versions, use the following `gem` command:
...
Refer to the https://docs.fedoraproject.org[Fedora docs] if you are unsure.
...
You should see information about the Asciidoctor version and your Ruby environment printed in the terminal.
  ...
  Asciidoctor 1.5.0.dev [http://asciidoctor.org]
...
You use the `asciidoctor` command to run Asciidoctor from the command line.
